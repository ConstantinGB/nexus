...
| Module | Description |
|--------|-------------|
| **Git** | Manage GitHub, self-hosted, or local git repositories — 6-step setup wizard, pull/push/commit per repo |
| **LocalAI** | Set up and run local AI models — Claude detects hardware, generates install script, live inference UI |
| **Custom** | AI-first open project — CLAUDE.md as system context, conversational AI chat, user-defined shell command buttons |
| **Web** | Dev/Build/Test/Lint via your package manager; auto-detects framework from `package.json` |
| **Research** | Note list with New Note, ripgrep Search, and URL export; configurable notes directory |
| **Codex** | Zettelkasten knowledge base — new notes with frontmatter skeleton, search, open vault |
| **Journal** | LaTeX journal entries — new entry from template, compile with pdflatex |
| **Game** | Godot project dashboard — game name, scene count, Launch Editor, Run Game, gdtoolkit lint |
| **Org** | Plan/diagram/schedule creator — Mermaid and Markdown table templates, file list by mtime |
| **Home** | Home Assistant dashboard — ping HA, call API, YAML config file list |
| **Streaming** | OBS dashboard — scene collection list, Launch OBS, tail logs |
| **VTube** | Virtual avatar pipeline display (camera → tracker → runtime → OBS), launch controls |
| **Emulator** | ROM library browser by system with counts, Launch RetroArch, Browse by System |
| **Vault** | Tool inventory (gpg/age/veracrypt/keepassxc-cli), GPG key management, age file encryption |
| **Server** | Service dashboard with live status (systemd + docker), Start/Stop/Logs per service |
| **Backup** | Encrypted, deduplicated backups via restic — local, NAS (SFTP), or NFS; snapshot browser, restore, integrity check |
...
| **Org** | `org_list_plans`, `org_new_plan`, `org_new_diagram` |
...
| **Server** | `server_list_services`, `server_status`, `server_start`, `server_stop` |
...
- **Backup paths**: restic repo and source paths are tilde-expanded before use; the repo is auto-initialised on first backup so no manual `restic init` is needed.
